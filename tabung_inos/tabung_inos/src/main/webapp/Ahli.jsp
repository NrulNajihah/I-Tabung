<%-- 
    Document   : Ahli
    Created on : 26 Sep 2024, 11:28:37 am
    Author     : NAJIHAH
--%>
    ...
    <body>
                ...
                <div class="container">
                    <div class="page-inner">
                        <div class="page-header">
                            <h3 class="fw-bold mb-3">Ahli Projek</h3>
                            <ul class="breadcrumbs mb-3">
                                ...
                                <li class="nav-item">
                                    <a href="<%=request.getContextPath()%>/AhliServlet?path2=listMmb&id=${project.id}">Senarai Ahli</a>
                                </li>
                            </ul>
                        </div>
                        <div class="row">
                                    ...
                                    <div class="card-body">
                                        <div class="table-responsive">
                                            <table
                                                ...
                                                <tbody>
                                                    <c:forEach var="member" items="${ListMmb}">
                                                        <tr>
                                                            <td><c:out value="${member.username}" /></td>
                                                            <td><c:out value="${member.position}" /></td>
                                                            <td><div class="form-button-action">
                                                                    <a href="AhliServlet?path2=deleteMmb&mID=<c:out value='${member.mID}'/>&id=${project.id}">
                                                                        <button
                                                                            type="button"
                                                                            title="Remove"
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="card-title">Daftar Ahli</div>

                                </div>
                                <c:if test="${member == null}">
                                    <form action="AhliServlet" method="post">
                                        <input type="hidden" name="path2" value="insertMmb">
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="form-group">
                                                    <label for="defaultSelect">Nama Ahli</label>
                                                    <select class="form-select form-control" name="uID" style="width:45rem;">
                                                        <c:forEach var="user" items="${listofUser}">
                                                            <option value="${user.uID}" <c:if test="${member != null && user.uID == member.uID}">selected</c:if>>
                                                                ${user.username}
                                                            </option>
                                                        </c:forEach>
                                                    </select>
                                                ...
                                                <div class="form-group">
                                                    <!-- Corrected the EL syntax for conditionally assigning the value -->
                                                    <input type="hidden" class="form-control form-control-lg" name="position"
                                                           value="${member != null && member.position != null ? member.position : 'Ahli'}" />
                                                </div>
                                            </div>
                                        </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const insertMmb = '<%= session.getAttribute("insertMmb") != null ? session.getAttribute("insertMmb") : "" %>';
                const deleteMmb = '<%= session.getAttribute("deleteMmb") != null ? session.getAttribute("deleteMmb") : "" %>';
                const loginError = '<%= session.getAttribute("loginError") != null ? session.getAttribute("loginError") : "" %>';

                if (insertMmb === "AddMmb") {
                    alert('Successfully Add Member!');
            <% session.removeAttribute("insertMmb"); %>
                } else if (deleteMmb === "Remove") {
                    alert('Successfully Remove Member!');
            <% session.removeAttribute("deleteMmb"); %>
                } else if (loginError) {
                    alert(loginError);
