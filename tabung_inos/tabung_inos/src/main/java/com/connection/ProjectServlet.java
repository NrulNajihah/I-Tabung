...
public class ProjectServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("ProjectServlet ==> Action3 = " + action);
        ...
        try {
            switch (action) {
                ...
                case "listReport":
                    ...
                    break;
                case "HomeStatus":
                    retrieveStatus(request, response);
                    break;
                case "fetchProjects":
                    if (uIDStr != null && tIDStr != null) {
                        int uID = Integer.parseInt(uIDStr);
                        int tID = Integer.parseInt(tIDStr);
                        fetchProjects(request, response, tID);
                    }
                    break;
                case "fetchChart":
                    if (uIDStr != null && tIDStr != null) {
                        int uID = Integer.parseInt(uIDStr);
                        int tID = Integer.parseInt(tIDStr);
                        fetchChart(request, response, tID);
                    }
                    break;
                default:
                    retrievePro(request, response);
    ...
    private void displayNewProForm(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatcher = sc.getRequestDispatcher("/ProjectServlet?action3=listPro");
    ...
    private void createPro(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.sendRedirect(request.getContextPath() + "/ProjectServlet?action3=listPro");
    ...
    private void retrievePro(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Project> listProject = projectDAO.selectAllProjectsByUserId(uID);
    ...
    private void retrieveProE(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession();
        Integer uID = (Integer) session.getAttribute("uID");  // Get uID from session
        System.out.println("uID: " + uID);
        if (uID == null) {
            response.sendRedirect("signIn.jsp");
            return;
        ...
        List<Project> listofProject = projectDAO.selectProjectsByUserMembership(uID);

        Date currentDate = new Date(); // Current date for status determination

        // Set the status for each project based on the current date
        ...
        request.getRequestDispatcher("/ListP.jsp").forward(request, response);
    }
    ...
    private void retrieveProI(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("listofProject", listofProject);
        request.getRequestDispatcher("/ListPer.jsp").forward(request, response);
    ...
    private void retrieveStatus(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ...
        List<Project> listTotal = projectDAO.selectTotal(uID);
    ...
    private void fetchProjects(HttpServletRequest request, HttpServletResponse response, int tID)
            throws ServletException, IOException {
        HttpSession session = request.getSession();
        ...
        List<Project> projectList = projectDAO.selectAllProtID(uID, tID);
    ...
    private void fetchChart(HttpServletRequest request, HttpServletResponse response, int tID)
            throws ServletException, IOException {
        ...
        Integer uID = user.getuID();
        List<Project> projectList = projectDAO.selectAllProCtID(uID, tID);
    ...
    private void updateDate(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("uID: " + uID);
            if (uID == null) {
                request.getSession().setAttribute("loginAgain", "sign");
            response.sendRedirect("signIn.jsp");
            return;
        }
            userDAO usrDAO = new userDAO(DbCon.getConnection());
