...
public class ExpensesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String path = request.getParameter("path4");

        System.out.println("ExpensesServlet ==> path4 = " + path);
    ...
    private void displayNewExpForm(HttpServletRequest request, HttpServletResponse response)
        ...
        if (IDStr != null && !IDStr.isEmpty()) {
            ...
            RequestDispatcher dispatcher = sc.getRequestDispatcher("/Expenses.jsp");
    ...
    private void displayEditExpForm(HttpServletRequest request, HttpServletResponse response)
        ...
        if (IDStr != null && !IDStr.isEmpty()) {
            ...
            ServletContext sc = getServletContext();
            RequestDispatcher dispatcher = sc.getRequestDispatcher("/editExps.jsp");
            dispatcher.forward(request, response);
        } else {
            response.sendRedirect("error.jsp");
        }
...
// Check if the sum of expenses exceeds total_All
            ...
            response.sendRedirect(request.getContextPath() + "/ExpensesServlet?path4=listExp&id=" + id);
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(ExpensesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    private void createExpH(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, SQLException, ParseException {
        try {
            HttpSession session = request.getSession();
            Integer uID = (Integer) session.getAttribute("uID");
            ...
            if (cIdStr == null || cIdStr.isEmpty()) {
                throw new ServletException("cID parameter is missing or empty");
            }
            ...
            // Create and insert the expense
            Expense expense = new Expense(uID, cID, claim, edate, noPO, total_Exp, receipt, filePart != null ? filePart.getSubmittedFileName() : null);
            expDAO.insertExpense(expense);

            

            System.out.println("Expense created for ID: " + cID);
    ...
    private void retrieveExpFull(HttpServletRequest request, HttpServletResponse response)
        ...
        // Forward the request to the JSP page for display
        request.getRequestDispatcher("/FullExp.jsp").forward(request, response);
    }
...
// Helper method to create a table cell with styling
    ...
    private void updateExp(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Expense expense = new Expense(eID, id, vID, claim, edate, noPO, total_Exp);  // Ensure Expense class has these fields
            System.out.println("Expense object before update: eID=" + expense.geteID() + ", id=" + expense.getId()
                    ...
                    + ", noPo=" + expense.getNoPO()
                    + ", edate=" + expense.getEdate());

            boolean isUpdated = expDAO.updateExpNo(expense);  // Update the method in DAO to handle these fields
            ...
            if (total_Exp.compareTo(total_All) > 0) {
                ...
                request.getRequestDispatcher("/ExpensesServlet?path4=newExp&id=" + id).forward(request, response);
            ...
            response.sendRedirect(request.getContextPath() + "/ExpensesServlet?path4=listExpFull&id=" + id);
        } catch (SQLException ex) {
            Logger.getLogger(ExpensesServlet.class.getName()).log(Level.SEVERE, "SQL error occurred", ex);
    ...
    private void deleteExp(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ExpensesServlet.class.getName()).log(Level.SEVERE, "SQL error occurred", e);
